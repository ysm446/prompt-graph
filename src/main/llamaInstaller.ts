// llama.cpp リリースの取得・ダウンロード・展開。
// lm-graph (src/main/llamaInstaller.ts) を流用し、本アプリ向けに調整。
...
function buildRelease(release: GithubRelease): LlamaRelease | null {
  ...
  variants.sort(
    (left, right) =>
      familyRank(left.family) - familyRank(right.family) || left.label.localeCompare(right.label)
  )
...
export async function installLlamaVariant(input: {
  ...
  try {
    ...
    const serverZip = join(tmpdir(), `prompt-graph-${randomUUID()}.zip`)
    ...
    if (variant.cudartUrl) {
      const cudartZip = join(tmpdir(), `prompt-graph-${randomUUID()}.zip`)
