...
## 2026-07-02 (モデル選択バーを lm-graph 準拠のモーダル方式に)

- `components/ModelBar.tsx` を再設計。select ドロップダウン + Load/Eject ボタン → **中央のモデルセレクタボタン + モーダルで選択**（lm-graph 準拠）。
  - セレクタボタン: Cpu アイコン + モデル名 + シェブロン。未ロード=グレー / ロード済み=紫アクセント / ロード中=紫パルス + スピナー。
  - モーダル: モデル行をクリックすると**即ロード**（`llamaServer.start()` が内部で stop するので稼働中の切替も安全）。行に vision バッジ / params / quant / サイズを表示、ロード中の行は紫ハイライト。ヘッダーに再読込・アンロード・baseUrl・Close。Escape / 外側クリックで閉じる。
  - ランタイム導入・設定ポップオーバーはトークンベースに再スタイル（挙動は不変）。
- 検証: typecheck / build 通過。実起動でモーダル表示（バッジ・ハイライト・レイアウト）をスクリーンショット確認。
